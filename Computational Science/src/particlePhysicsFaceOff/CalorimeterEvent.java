...
public class CalorimeterEvent implements Serializable {
	...
	private final double closeArea = 0.5;
	
	//variables for level 1 and 2
	private double finalWidth;
	private double finalError;
	private double singleEnergy;
	...
	/**
	 * 
	 * @return single energy cluster
	 */
	public double getSingleEnergy(){
		double energy = 0;
		int count = 0;
		for(Tower s: towers){
			energy += s.getEnergy();
	...
	public void getWidth(){
		...
		double width = 0;
		for (double i = 0; i < 2; i += widthIncrement){
	...
	/**
	 * @return tower to energy hashmap
	 */
	public Map<Tower, Double> getEnergyMap(){
		Map<Tower, Double> energies = new HashMap<Tower, Double>();
		for(Tower t: seedTowers){
			double distance = getWidthMap().get(t);
			double energy = 0;
			for(Tower s: closeTowers(t)){
				energy += s.getEnergy();
			}
			energies.put(t, energy);
		}
		return energies;
	}
	
	/**
	 * @return Arraylist of sorted energies
	 */
	public List<Double> getSortedEnergies(){
	 ...
	 * @return Arraylist of sorted energies
	 */
	public Map<Tower, Double> getSortedMapEnergies(){
		Map<Tower, Double> energies = new HashMap<Tower, Double>();
		for(Tower t: seedTowers){
			double energy = 0;
			if(0 <= t.getPhi() && t.getPhi() <= 2*Math.PI){
	...
	/**
	 * Prints Energies Map to Console
	 */
	public void printEnergyMapToConsole(){
		for(Tower t: seedTowers){
			if (0 <= t.getPhi() && t.getPhi() <= 2*Math.PI){
				System.out.println(t.getEta() + "\t" + t.getPhi() + "\t" + getEnergyMap().get(t));
			}
		}
	}
	/**
	...
	public String toString(){
		return highestEnergy.getEnergy() + "\t" + getSingleEnergy() + "\t" + finalWidth;
	}
}
