...
abstract public class Animal implements Drawable {
	...
	public Animal() {
		...
		male = Arena.getRandom().nextBoolean();
	}
	...
	protected boolean checkMateability(Animal other) {
		return sexuallyMature() && other.sexuallyMature() && !isPregnant() && !other.isPregnant()
				&& isMale() != other.isMale()
				&& checkType(other) && getCell().getX() == other.getCell().getX() && getCell().getY() == other.getCell().getY();
	}
	...
	protected Animal createOffspring() {
		try {
			Animal returntype = getClass().newInstance();
			returntype.addEnergy(initialEnergy());
			return returntype;
	...
	public boolean makeMove(Direction dir) {
		...
		case LEFT:
			if (current.getX() > 0) {
				move(current.getMap().getCell(current.getX() - 1, current.getY()));
				return true;
			} else {
				return false;
			}
		...
		case UP:
			if (current.getY() > 0) {
				move(current.getMap().getCell(current.getX(), current.getY() - 1));
				return true;
			} else {
				return false;
			}
