...
public class World {
	...
	private boolean magneticForce;
	private Vector magneticField;
	
	public World(double C, Vector g, double k, double floor, Vector magneticField)
	{
		...
		for (Projectile p: projectiles)
		{
			...
			while(p.getPosition().getZ() >= floor)
			{
				...
				if (magneticForce)
				{
					if (p.getVelocity().getMagnitude() != 0)
					{
						//direction is cross product of velocity and field
						Vector direction = Vector.crossProduct(p.getVelocity(), magneticField);
						p = p.addForce(Vector.scaleVector(direction, p.getVelocity().getMagnitude() * 10 * 0.01));
					}
				}
