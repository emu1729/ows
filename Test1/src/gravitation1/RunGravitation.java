...
public class RunGravitation {
	...
	private static final int nIncrements = 1;
	...
	private static final double massRatio = Math.PI/4;
	...
	public static void main(String[] args){ //throws FileNotFoundException{
		...
		//level2();
		//level2PartII();
		for (int i = 500000; i < 520000; i += 1000)
		{
	...
	public static void level3(int thickness)
	{
		GravitationEngine universe = new GravitationEngine(System.out);
		ExtendedProjectile earth = new ExtendedProjectile("Earth", false);
        for(int i = 0; i < earthPartitions; ++i) {
            for(int j = 0; j < earthPartitions; ++j) {
                for(int k = -4; k <= 0; k += 1) {
                    ...
                    double z = k * thickness/4;
                    //System.out.println(x + "\t" +  y + "\t" + z);
					if (x*x+y*y <= EARTH_RADIUS*EARTH_RADIUS)
					{
						earth.addProjectile(new PointProjectile(new Vector3(x, y, z), new Vector3(), EARTH_MASS / (Math.pow(earthPartitions,2)*Math.PI), ""));
					}
                }
            }
        }
        universe.addProjectile(earth);
		universe.addProjectile(new PointProjectile(new Vector3(0, 0, 1), new Vector3(0, 0, 0), 20, "Moon"));
		//System.out.println(universe.header());
		universe.run(timeIncrement, nIncrements, true);
	}
